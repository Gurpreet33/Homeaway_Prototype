...
import { ownerDashboard } from "../../actions/ownerdashboardActions";
...
class OwnerDashboard extends Component {
  ...
  componentDidMount() {
    ...
    this.props.ownerDashboard(data, this.props.history);
  }
  handleInboxOwner = e => {
  ...
  render() {
    ...
    const properties = Object.keys(this.props.properties)
      .map(property => this.props.properties[property])
      .slice(0, 5);
    const propertiesTotal = Object.keys(this.props.properties).map(
      property => this.props.properties[property]
    );
      ...
      <React.Fragment>
        <div>
          ...
          <div>
            ...
            >
              {user.name}, here are your properties
            </h2>
...
const mapStateToProps = state => {
  return {
    ...
    properties: state.ownerdashboardReducer.properties
  };
};

export default connect(
  mapStateToProps,
  { ownerDashboard, logoutUser }
)(withRouter(OwnerDashboard));
